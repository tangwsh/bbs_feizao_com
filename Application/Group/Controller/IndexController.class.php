...
class IndexController extends GroupController
{
     ...
     * @auth 陈一枭
     */
    public function group($id = 0, $page = 1, $order = '', $type = 'post', $r = 20, $title = '', $cate = '')
    {
        $id = intval($id);
        $page = intval($page);
        $order = op_t($order);
    ...
    public function groups($order = 'create_time', $cate = 0, $page = 1, $r = 5, $keywords = '', $uid = 0)
    {
        ...
        if ($order == 'activity') {
            ...
            $count = $Model->query("SELECT group_id,count(group_id) as count from thinkox_group_member group by group_id order by count desc");
    ...
    public function doAddGroup($group_id = null, $group_type, $title, $detail, $logo, $type, $background)
    {
        ...
        $logo_size = getimagesize(is_local()?$server_host . get_cover($logo, 'path'):get_cover($logo, 'path'));

        if ($logo_size[0] < 300 || $logo_size[1] < 300) {
            $this->error('logo尺寸至少为300*300');
        }
