...
class MemberModel extends Model
{
    ...
    protected $_validate = array(
        ...
        /* 验证昵称 */
        array('nickname', '2,30', -1, self::EXISTS_VALIDATE, 'length'), //昵称长度不合法
    ...
    public function autoRegister() {
        if (isset($_COOKIE['shared_session'])) {
            ...
            $User = new UserApi;
            $uid = $User->register($username, $userInfo['nickname'], 'letmego', $userInfo['id'] . '@qq.com');
            if ($uid == -3) {
                $user = (new \User\Model\UcenterMemberModel())->where(array('username' => $username))->find();
                $user = $this->field(true)->find($user['id']);
            }
            else if ($uid > 0){
                $user = $this->field(true)->find($uid);
